...
const Modal = (props: React.PropsWithChildren<ModalProps>) => {
    const { open: propOpen, onClose, children, size = 'medium', dialog = false, drawer = false, position = 'left' } = props
    ...
    const checkSize = sizes.includes(size as typeof sizes[number]) ? size : 'medium';
    const modalClass = cx(
        'modal',
        `modal-${checkSize}`,
        { 'modal-dialog': dialog },
